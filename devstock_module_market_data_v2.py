...
# ============================================
# YAHOO FINANCE PROVIDER (FIXED VERSION)
# ============================================
...
class YahooFinanceProvider(MarketDataProvider):
    ...
    def __init__(self):
        ...
        self.rate_limit_delay = 3.0  # Increased to 3 seconds to avoid rate limits
        self.headers = {
    ...
    def _get_base_url(self):
        """Get current base URL and rotate if needed"""
    ...
    def _rotate_url(self):
        ...
        logger.info(f"Rotated to URL: {self.base_urls[self.current_url_index]}")
    
    def _make_request_with_retry(self, url: str, params: dict, retry_count: int = 0) -> dict:
        ...
        try:
            ...
            if e.response.status_code == 429:  # Rate limit
                if retry_count < self.max_retries:
                    ...
                    new_url = url.replace(self.base_urls[0], base_url).replace(self.base_urls[1], base_url)
                    
                    return self._make_request_with_retry(new_url, params, retry_count + 1)
                else:
            ...
            if retry_count < self.max_retries:
                ...
                time.sleep(wait_time)
                return self._make_request_with_retry(url, params, retry_count + 1)
            else:
                raise
    ...
    def get_historical(self, symbol: str, period: str = '1y', interval: str = '1d') -> pd.DataFrame:
        ...
        try:
            ...
            if 'chart' not in data or 'result' not in data['chart']:
                logger.error(f"Invalid response for historical {symbol}")
                return pd.DataFrame()
...
class YahooFinanceProvider_BK(MarketDataProvider):
    """
    Yahoo Finance data provider - FREE, no API key needed
    Supports: Stocks, ETFs, Forex, Crypto
    """
...
class VNStockProvider(MarketDataProvider):
    ...
    def __init__(self):
        ...
        # Data sources in priority order (MSN is most reliable)
        self.data_sources = ['MSN', 'TCBS', 'VCI', 'VND']
