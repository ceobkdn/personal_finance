...
class MonthlyFinanceTracker:
    ...
    def create_widgets(self):
        ...
        self.expense_category = widgets.Dropdown(
            options=['Ăn uống', 'Đi lại', 'Nhà ở', 'Giải trí', 'Mua sắm', 'Gửi về VN',
        ...
        self.savings_type = widgets.Dropdown(
            options=['Tiết kiệm ngân hàng', 'Đầu tư chứng khoán', 'Quỹ đầu tư', 'Quỹ cân bằng', 'Quỹ cổ phiếu', 'Quỹ trái phiếu',
    ...
    def create_layout(self):
        ...
        income_box = widgets.VBox([
            self.income_header,
            widgets.HBox([self.income_source]),
            widgets.HBox([self.income_amount, self.income_currency]),
            widgets.HBox([self.income_date]),
            widgets.HBox([self.add_income_btn],layout=widgets.Layout(justify_content='center'))
        ])
        
        # Expense section
        expense_box = widgets.VBox([
            self.expense_header,
            widgets.HBox([self.expense_category]),
            widgets.HBox([self.expense_description]),
            widgets.HBox([self.expense_amount, self.expense_currency]),
            widgets.HBox([self.expense_date]),
            widgets.HBox([self.add_expense_btn],layout=widgets.Layout(justify_content='center'))
        ])
        ...
        loan_box = widgets.VBox([
            ...
            widgets.HBox([self.loan_type]),
            widgets.HBox([self.loan_description]),
            widgets.HBox([self.loan_total, self.loan_currency]),
            widgets.HBox([self.loan_monthly]),
            widgets.HBox([self.loan_paid]),
            widgets.HBox([self.loan_date]),
        ...
        savings_box = widgets.VBox([
            ...
            widgets.HBox([self.savings_type]),
            widgets.HBox([self.savings_description]),
            widgets.HBox([self.savings_amount, self.savings_currency]),
            widgets.HBox([self.savings_date]),
        ...
        tab1a = widgets.VBox([widgets.HBox([income_box])])
        tab1b = widgets.VBox([widgets.HBox([expense_box])])
        #tab2 = widgets.VBox([widgets.HBox([loan_box, savings_box])])
        tab2a = widgets.VBox([widgets.HBox([loan_box])])
        tab2b = widgets.VBox([widgets.HBox([savings_box])])
        tab3 = analysis_box
        
        tabs = widgets.Tab()
        tabs.children = [tab0, tab1a, tab1b, tab2a, tab2b, tab3]
        tabs.titles = ['Cài đặt', 'Khoản thu', 'Khoản chi', 'Vay Nợ',  'Đầu tư', 'Phân Tích']
    ...
    def delete_savings(self, button):
        ...
        for saving in self.savings_data:
            ...
            if key == selected_key:
                ...
                if deleted_currency is None:
                    deleted_currency = saving['currency']
            else:
        ...
        if deleted_items:
            ...
            # Calculate total deleted amount với đơn vị gốc (không chuyển đổi)
            total_deleted = 0
            for item in deleted_items:
                total_deleted += item['amount']
            
            # Tách phần hiển thị tên từ key để loại bỏ currency
            display_name = selected_key.split(' (')[0]  # Lấy phần trước dấu ngoặc
            
            with self.output:
                clear_output()
                print(f"🗑️ Đã xóa {len(deleted_items)} giao dịch: {display_name}")
                print(f"   💰 Tổng số tiền đã xóa: {self.format_currency(total_deleted, deleted_currency)}")
                print(f"   💱 Loại tiền tệ: {deleted_currency}")
                print("💾 Dữ liệu đã được lưu tự động")
        else:
            with self.output:
                clear_output()
                print("❌ Không tìm thấy giao dịch nào để xóa!")
