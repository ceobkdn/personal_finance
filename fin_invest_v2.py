...
class InvestmentPortfolioAnalyzer:
    ...
    def show_allocation_analysis(self, button):
        ...
        with self.output:
            ...
            # Risk assessment
            print(f"\n⚠️  ĐÁNH GIÁ RỦI RO PHÂN BỔ:")
            
            # Check for over-concentration
            max_allocation = max(current_allocation_pct.values()) if current_allocation_pct else 0
            if max_allocation > 50:
                print(f"   🔴 Tập trung quá cao: Một loại tài sản chiếm {max_allocation:.1f}%")
            elif max_allocation > 40:
                print(f"   🟡 Hơi tập trung: Loại tài sản lớn nhất chiếm {max_allocation:.1f}%")
            else:
                print(f"   🟢 Phân bổ hợp lý: Không có loại tài sản nào quá lớn")
            
            # Check target allocation completeness
            target_total = sum(self.target_allocation.values())
            if target_total != 100:
                print(f"   ⚠️  Tổng tỷ lệ mục tiêu: {target_total:.1f}% (khuyến nghị: 100%)")
